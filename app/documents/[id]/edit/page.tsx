...
export default function EditDocumentPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/documents/${id}`)
      ...
      .then((d: DocumentData) => {
        setDocType(typeConfig[d.type] ? d.type : 'invoice')
        setDocNo(d.no || '')
        let parsed: Item[] = []
        try {
          const raw = typeof d.items === 'string' ? JSON.parse(d.items) : d.items
        ...
        if (parsed.length === 0) parsed = [{ name: '', detail: '', qty: 1, unit: 'งาน', price: 0 }]
        setForm({
          customerId: d.customerId || '',
          quotationId: d.quotationId || '',
          refInvoiceId: d.refInvoiceId || '',
          issueDate: d.issueDate || today,
          dueDate: d.dueDate || '',
          items: parsed,
          discount: d.discount || 0,
          vatEnabled: !!d.vatEnabled,
          bankIndex: d.bankIndex || 0,
          payMethod: d.payMethod || 'โอนเงิน',
  ...
  function handleQuotationChange(qid: string) {
    const q = quotations.find(x => x.id === qid)
    if (!q) { setField('quotationId', qid); return }
    let items: Item[] = []
    ...
    try {
      await fetch(`/api/documents/${id}`, {
        ...
        body: JSON.stringify({
          ...
          bankIndex: Number(form.bankIndex),
  ...
  if (notFound) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 300, color: '#7a8893', gap: 12 }}>
        ...
        <div style={{ fontSize: 16, fontWeight: 600 }}>ไม่พบเอกสาร</div>
        <Link href="/documents" style={{ color: '#5f7d99', fontSize: 14 }}>← กลับไปรายการ</Link>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 13, color: '#9aa7b2', margin: '16px 0 6px' }}>
        ...
        <Link href={`/documents/${id}`} style={{ color: '#9aa7b2', textDecoration: 'none', fontFamily: "'IBM Plex Sans', monospace" }}>{docNo}</Link>
        <span className="material-symbols-rounded" style={{ fontSize: 16 }}>chevron_right</span>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 14, margin: '0 0 18px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 13 }}>
          ...
          <div>
            <div style={{ fontSize: 22, fontWeight: 700, color: '#2f3b45' }}>แก้ไข{cfg.label}</div>
            <div style={{ fontSize: 13, color: '#9aa7b2', fontFamily: "'IBM Plex Sans', sans-serif" }}>{docNo}</div>
          </div>
        </div>
      ...
      <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap', alignItems: 'flex-start' }}>
            ...
            <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
              ...
              <div style={{ flex: 1, minWidth: 200 }}>
                <div style={{ ...fieldLabel, display: 'flex', alignItems: 'center', gap: 8 }}>
                  {refLabel}
                  {docType === 'receipt' && form.refInvoiceId && (
                    <Link href={`/documents/${form.refInvoiceId}`} style={{ color: '#5f7d99', textDecoration: 'underline', textUnderlineOffset: 2, fontWeight: 600 }}>เปิดดู</Link>
                  )}
                  {docType !== 'receipt' && form.quotationId && (
                    <Link href={`/quotation/${form.quotationId}`} style={{ color: '#5f7d99', textDecoration: 'underline', textUnderlineOffset: 2, fontWeight: 600 }}>เปิดดู</Link>
                  )}
                </div>
                {docType === 'receipt' ? (
        ...
        <div style={{ width: 320, flex: '1 1 280px', position: 'sticky', top: 6, background: '#fff', borderRadius: 18, border: '1px solid #edf0f3', padding: 24 }}>
          ...
          <div onClick={handleSave} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, height: 46, borderRadius: 12, background: '#5f7d99', color: '#fff', fontSize: 14.5, fontWeight: 600, cursor: saving ? 'wait' : 'pointer', marginTop: 20, boxShadow: '0 4px 12px rgba(95,125,153,.3)', opacity: saving ? 0.7 : 1 }}>
            <span className="material-symbols-rounded" style={{ fontSize: 20 }}>save</span>{saving ? 'กำลังบันทึก...' : 'บันทึกการแก้ไข'}
          </div>
